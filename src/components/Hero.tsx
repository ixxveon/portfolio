  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
        ...
        >
          {skills.map((group) => (
            <div key={group.category} className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
              ...
              <div className="flex flex-wrap gap-2">
                {group.items.map((item) => (
                  <span
                    key={item.name}
                    className="text-xs px-3 py-1.5 bg-gray-50 border border-gray-200 text-gray-700 rounded-full font-medium hover:bg-indigo-50 hover:border-indigo-200 hover:text-indigo-700 transition-colors"
                  >
                    {item.name}
                  </span>
                ))}
